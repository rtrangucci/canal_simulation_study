functions {
  ...
  matrix[] kernel_list(matrix[] d, real rho) {
    int N_seg = size(d);
    matrix[cols(d[1]), rows(d[1])] kernels[N_seg];
    for (seg in 1:N_seg)
  ...
  vector gen_mu(real[] pts, real ell, vector cond_vals, vector eta, real mag) {
    int S = size(pts);
    int Sm2 = S - 2;
    matrix[S, S] K = cov_exp_quad(pts, mag, ell);
    matrix[2, 2] K_22_inv = inv_22(K[S,S-1]); 
    matrix[Sm2,2] K_12 = block(K,1,S - 1,Sm2, 2);
    matrix[Sm2, Sm2] K_cond = block(K,1,1,Sm2,Sm2) - quad_form(K_22_inv,K_12');// * inv_mag_sq;
    for (s in 1:Sm2) {
      K_cond[s,s] += 1e-6;
    }
...
data {
  ...
  int<lower=0, upper=1> y[J, G]; // Observations
  matrix[S,J] dists[N_seg]; // HH dist to integration points 
  real M;
  real C;
  real pts[N_seg,S];
  real delta_t[N_seg];
  matrix[N_seg, 2] endpt_coords;
  int<lower=1,upper=N_endpts> endpt_inds[N_seg, 2];
...
transformed data {
  int<lower=0, upper=G> Y[J];
  // int ell_idx[N_seg];
  real scale = 1.0;
  real pts_aug[N_seg, S + 2];
...
transformed parameters {
  ...
  {
    row_vector[2] c_5_67 = weighting * [exp_quad(M / 2., ell), exp_quad(C / 3, ell)];
    row_vector[2] c_2_35 = weighting * [exp_quad(M / 2., ell), exp_quad(2. * C / 3, ell)];
    real c_1_2 = exp_quad(M / 2., ell);
    real c_4_5 = exp_quad(M / 2., ell);
    end_pts[6:7] = eta_end_pts[6:7];
    end_pts[3] = eta_end_pts[3];
    end_pts[5] = c_5_67 * end_pts[6:7] + sqrt(1 - dot_self(c_5_67)) * eta_end_pts[5];
    end_pts[2] = c_2_35 * [end_pts[3],end_pts[5]]' + sqrt(1 - dot_self(c_2_35)) * eta_end_pts[2];
    end_pts[1] = c_1_2 * end_pts[2] + sqrt(1 - c_1_2^2) * eta_end_pts[1];
    end_pts[4] = c_4_5 * end_pts[5] + sqrt(1 - c_4_5^2) * eta_end_pts[4];
  }
  {
    matrix[J,S] k_dist[N_seg] = kernel_list(dists, rho);
    for (seg in 1:N_seg) {
    //  real ell_use = ell[ell_idx[seg]];
      f[seg] = gen_mu(pts_aug[seg], ell, end_pts[endpt_inds[seg,]], eta_f[,seg], 1.0);
...
model {
  for (j in 1:J) {
    ...
    target += Y[j] * log1m_exp(-risk)
            + (G - Y[j]) * -risk;
  }
  to_vector(eta_f) ~ normal(0, 1);
